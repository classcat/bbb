...
function createMeetings() {
	...
	if (nOfMeetings == 0) { 
                // ClassCat : 13-jan-15
		$("#no_meetings").text("現在使用中の会議室はありません。");
...
function updateMeetingInfo() {
	$.ajax({
    	type: "GET",
		url: 'cc-activity-monitor_helper.jsp?getxml=true',
		dataType: "text/xml",
		cache: false,
...
function createMeetingTable(meeting) {	
	...
	form += '</th>';
        ...
        // ClassCat : 13-jan-15
	var tableContent = '<table name="' + meeting.meetingID + '" class="hor-minimalist-b" cellspacing="0" summary="The current participants in a meeting"><caption style="color:orangered;padding:0 0 4px 0;font-weight:bold;">' + meeting.meetingID + '<caption><tr><th scope="col" abbr="Participants">参加者 ID</th><th scope="col" abbr="Name">名前</th><th scope="col" abbr="Role">ロール（役割）</th>';
	...
	//uncomment below to add the ability to end meetings in the activity monitor
        // ClassCat : 13-jan-15 : This functionality will require moderator's password.
	// tableContent += form;
